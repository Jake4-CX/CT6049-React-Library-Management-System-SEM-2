...
AFTER INSERT OR UPDATE ON loanedBooks
FOR EACH ROW
BEGIN
  IF INSERTING THEN
    IF :new.returnedAt IS NOT NULL THEN
      IF TRUNC(NVL(:new.returnedAt, SYSDATE)) - TRUNC(:new.loanedAt) >= 14 THEN
        INSERT INTO loanFines (loanId, fineAmount, paidAt) 
        VALUES (:new.id, (((TRUNC(NVL(:new.returnedAt, SYSDATE)) - TRUNC(:new.loanedAt)) - 14) * 0.5) + 1, CURRENT_TIMESTAMP);
      END IF;
    END IF;
  ELSIF UPDATING THEN
